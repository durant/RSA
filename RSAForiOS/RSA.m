...
+ (BOOL)PKCSVerifyBytesSHA256withRSA:(NSData*)plainData signData:(NSData*) signature key:(NSString *)publicKey
{
    SecKeyRef key = [RSA addPublicKey:publicKey];
...
+ (NSData*)PKCSSignBytesSHA256withRSA:(NSData*) plainData key:(NSString *) privateKey
{
    SecKeyRef key = [RSA addPrivateKey:privateKey];
    size_t signedHashBytesSize = SecKeyGetBlockSize(key);
